...
public class DeleteMacACLEntry extends BasePacket implements IPacket {
	private short entryNumbers;
	private MacAddress[] aclMacAddress;
	@Override
	public byte[] serialize() {
		byte[] delMacACLEntryBytes = null;
		if(entryNumbers != 0 && aclMacAddress.length != 0) {
			delMacACLEntryBytes = new byte[6*entryNumbers + 1];
		}
		ByteBuffer bb = ByteBuffer.wrap(delMacACLEntryBytes);
		bb.put((byte)entryNumbers);
		for(int i = 0; i < entryNumbers; i++) {
			bb.put(aclMacAddress[i].getBytes());
		}
		return delMacACLEntryBytes;
	}
